...
const WishlistItem=(isLoggedIn)=>{
  ...
  const handleAddToCart = async (image_id,image_url,price,name,descr,uemail) => {
    if(uemail===null){
          ...
          <div key={item.image_id} className='cart-item'>
            ...
            <div className='cart-details'>
              ...
              <p>Price: ${item.price}</p>
                ...
                <button onClick={() => handleAddToCart(item.image_id,item.image_url,item.price,item.name,item.descr,uemail)}>Add to cart</button>
          </div>
        ))}
      </div>
      <div className='last'>
